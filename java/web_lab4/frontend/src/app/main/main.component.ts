...
export class MainComponent {
  ...
  onSubmit() {
    ...
    this.http.post('http://localhost:8080/lab4/api/main/submit-point', payload).subscribe(
      (response: any) => {
 ...
 drawPoint(xCoord: number, yCoord: number, r: number, hit: boolean): void {
  ...
  onClear() {
      ...
      this.http.delete(`http://localhost:8080/lab4/api/main/clear/${authToken}`).subscribe(
        response => {
